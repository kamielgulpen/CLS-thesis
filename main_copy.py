...
def initialize_edges_links(df_edges,all_nodes, layer ,group_nodes, hash_dict, division, id_source = None, id_destination = None, source = None, destination = None, barabasi = False):     
    ...
    for row in df_edges.iterrows():
        ...
        initial_list.append(
            (
            hash_dict[f"{row['lft_dst']}, {row['etngrp_dst']}, {row['geslacht_dst']}, {row['oplniv_dst']}"],
            hash_dict[f"{row['lft_src']}, {row['etngrp_src']}, {row['geslacht_src']}, {row['oplniv_src']}"],
            int(row['n'])
            )
            )
    ...
    for row in initial_list:
        ...
        connections = round(connections/25)
        ...
        for _ in range(connections):
            ...
            while True:
                ...
                if len(group_nodes[src_group]) > 0 and len(group_nodes[dst_group]) > 0:

                    src_node = random.choices(group_nodes[src_group])[0]
                    # dst_node = random.choices(group_nodes[dst_group])[0]
                    ...
                    if barabasi:
                        ...
                        dst_node = random.choices(list(nodes),(nodes_weights))[0]
                ...
                if barabasi and dst_node != src_node and dst_node not in link_dictionary[src_node]:
                ...
                elif dst_node != src_node and dst_node not in link_dictionary[src_node]:

                    # Appends both nodes to lists
                    if symetric and (dst_node, src_node) not in set(links):
...
if __name__ == '__main__':
    ...
    for layer_number, layer in enumerate(layers):
        ...
        df_.to_csv(f'Data/NW_data/{layer}_nw_b={barabasi}_small.csv')
