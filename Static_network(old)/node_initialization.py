...
#             if len(areas[area][group]) < math.ceil(areas_df[area][group] * sum(areas_df[area].values())) and area < len(area_list) :
...
from static_network.node import Node
...
def initialize_nodes(df, hash_dict):
    ...
    for i in range(df.shape[0]):
        ...
        for _ in range(int(group['n'])):
            ...
            if hh_1 and np.random.uniform() < 0.35 and hh_count < single_hoseholds:
                hh_count +=1
                
                node = Node(id, hd, hh_count)
            else: 
                node = Node(id, hd, 0)
